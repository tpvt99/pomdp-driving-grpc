...
if __name__ == "__main__":
    ...
    for i, key in enumerate(result):
        ...
        plt.xlabel("Argoverse")
        plt.ylabel("Summit")
        plt.title(f"Comparison of {key} between Argoverse and SUMMIT")
        ...
        for j in range(len(x)):
            bias_x = -random.uniform(0.03, 0.05)  # Randomize bias to avoid overlapping
            bias_y = random.uniform(0.02, 0.03)
            color = colors[j % len(colors)]  # Assign a color from the predefined set
            plt.scatter(x[j], y[j], color=color)  # Set the scatter point color
            plt.annotate(methods[j], xy=(x[j], y[j]), xytext=(x[j]+bias_x, y[j]+bias_y),
                 color=color, bbox=dict(facecolor='white', edgecolor='none', alpha=0.5, pad=0))
        ...
        plt.plot(x[:,1], model.predict(x), label="Best-fit Line")
        ...
        plt.savefig(f'Argoverse_VS_Summit_{key}.pdf')
        plt.close(fig)
