...
def plot_dynamic_vs_staticade_scatter(prediction_performance_map, methods_to_plot):
    ...
    for k, v in remap_methods.items():
        prediction_performance_map[v] = prediction_performance_map[k]
        del prediction_performance_map[k]
    ...
    dynamic_ade_dict = {'ade_predlen30': np.zeros(len(methods_to_plot)), 
    ...
    for key in prediction_performance_map.keys():
        ...
        for which_dynamic_ade in dynamic_ade_dict.keys():
            dynamic_ade_dict[which_dynamic_ade][idx] = np.nanmean(prediction_performance_map[key][which_dynamic_ade])
        print(f"Mean ADE {key} is {np.nanmean(prediction_performance_map[key]['ade_predlen30']):.2f} "
              f"ADE ADE predlen30 {key} is {np.nanmean(prediction_performance_map[key]['ade_obs20_predlen30']):.2f} " 
              f"ADE obs20 {key} is {np.nanmean(prediction_performance_map[key]['ade_20meters_closest_predlen30']):.2f}")
              #f"ADE closest {key} is {np.nanmean(prediction_performance_map[key]['ade_obs20_20meters_closest_predlen30']):.2f}")
    ...
    for which_dynamic_ade in dynamic_ade_dict.keys():
        ...
        for i, method in enumerate(methods_to_plot):
            plt.scatter(performance_summit[i], dynamic_ade_dict[which_dynamic_ade][i], color='black', label=method)
            plt.annotate(method, (performance_summit[i], dynamic_ade_dict[which_dynamic_ade][i]), 
                        textcoords="offset points", xytext=(-2,7), ha='center')

        # Add labels and legend
        plt.xlabel(f'Static ADE', fontsize=12)
        plt.ylabel(f'Dynamic ADE', fontsize=12)
        plt.title("Static ADE vs. Dynamic ADE of RVO Planner", fontsize=12)
        plt.ylim(top=max(dynamic_ade_dict[which_dynamic_ade])*1.02)  # Adjust the ylim to provide more space for the legend
        plt.xlim(left = min(performance_summit) * 0.95, right=max(performance_summit)*1.03)

        ax = plot_best_fit_line(performance_summit, dynamic_ade_dict[which_dynamic_ade], ax)

        plt.tight_layout()
        plt.grid()
        plt.savefig(f"3_Static_Dynamic_{which_dynamic_ade.upper()}_RVO.pdf", dpi=300)
        plt.clf()
        plt.close()

    # Plot all in 1 plot (Not good)
    #fig, ax = plt.subplots(figsize=(5, 5)) # adjust figure size as needed
    #colors = plt.cm.rainbow(np.linspace(0, 1, len(methods_to_plot)))
    #plt.rcParams.update({'font.size': 12})
    #j = 0
    ...
    #    for i, method in enumerate(methods_to_plot):
    #        plt.scatter(performance_summit[i], dynamic_ade_dict[which_dynamic_ade][i], color=colors[j], label=method)
    #        plt.annotate(method, (performance_summit[i], dynamic_ade_dict[which_dynamic_ade][i]), 
    #                    textcoords="offset points", xytext=(-2,7), ha='center', color=colors[j])


    #    ax = plot_best_fit_line(performance_summit, dynamic_ade_dict[which_dynamic_ade], ax, color=colors[j], color_text=colors[j])
    #    j += 1
    ...
    #plt.tight_layout()
    #plt.grid()
    #plt.savefig(f"3_Static_Dynamic_ADE_RVO.pdf", dpi=300)
    #plt.clf()
    #plt.close()
...
#        performance_summit = performance_summit_mfde
#
#    # 1/ This is the scatter plot with best-fit-line
...
#    #    performance_RVO_predlen30[idx] = np.nanmean(prediction_performance_RVO[key]['ade_predlen30'])
...
#    #    performance_RVO_20meters_closest[idx] = np.nanmean(prediction_performance_RVO[key]['ade_20meters_closest'])
#    #    print(f"Mean ADE {key} is {np.nanmean(prediction_performance_RVO[key]['ade']):.2f} "
#    #          f"ADE ADE predlen30 {key} is {np.nanmean(prediction_performance_RVO[key]['ade_predlen30']):.2f} " 
#    #          f"ADE obs20 {key} is {np.nanmean(prediction_performance_RVO[key]['ade_obs20']):.2f}"
#    #          f"ADE closest {key} is {np.nanmean(prediction_performance_RVO[key]['ade_closest']):.2f}"
#    #          f"ADE closest 20meters {key} is {np.nanmean(prediction_performance_RVO[key]['ade_20meters_closest']):.2f}")
#   
#
#    offsets=0.4
#    space_between_methods = 1.2
...
#    ax.bar(position-0*offsets, performance_RVO, width=offsets, color=(*colors.to_rgba(default_colors[2])[:3], 0.5), align='center', label='ADE (D)')
#    ax.bar(position+1*offsets, performance_RVO_closest, width=offsets, color=(*colors.to_rgba(default_colors[3])[:3], 0.5), align='center', label='ADE_closest (D)')
...
#    ax.set_title('Comparison of Static_ADE vs. Dynamic_ADE of RVO')
#    ax.set_ylabel('ADE')
#    #ax.legend(loc='upper right', fontsize='small')
#    ax.legend(ncol=5, loc='upper center', fontsize='small')
#    ax.set_ylim(0, 3.6)  # Adjust the ylim to provide more space for the legend
#
#    plt.tight_layout()
#    plt.savefig("3_Static_ADE_vs_Dynamic_ADE_RVO.png")
#    plt.clf()

def plot_best_fit_line(x, y, ax, color="black"):
    ...
    $p-value$: {p_value:.4f}
    """
    ax.text(0.05, 0.10, stats_text, transform=ax.transAxes, fontsize=12, verticalalignment='bottom', color=color)
...
def plot_static_ade_vs_driving_performance(prediction_performance_map, 
    ...
    for performance_metric in ['driving', 'safety', 'efficiency', 'comfort']:
        ...
        # Add labels and legend
        plt.xlabel(f'Static {metric}', fontsize=12)
        plt.ylabel(f'{performance_metric.capitalize()} Performance', fontsize=12)
        ...
        plt.xlim(right=max(performance_summit)*1.06)
        ...
        plt.title(f'RVO Planner')
        ax = plot_best_fit_line(performance_summit, [method_performance[x][performance_metric] for x in methods_to_plot], ax)

        plt.tight_layout()
        plt.grid(True, linewidth=0.5, alpha=0.7)

        plt.savefig(f"D_Static_{metric}_vs_{performance_metric.capitalize()}_Performance_RVO.pdf", dpi=300)
        plt.clf()
        plt.close()


def plot_dynamic_ade_vs_driving_performance(prediction_performance_map, 
                                            driving_performance_map, metric='ADE'):
    ...
    for best_fit_metric in prediction_performance_map['cv'].keys():
        ...
        driving_performance = [driving_performance[method]['driving'] for method in methods_to_plot]
    ...
    metric = 'ade_closest_predlen30'
    metric_text_name = 'ADE'
    driving_performance = get_method_performance(prediction_performance_map, driving_performance_map, 
                                                        methods_to_plot, 'ade')
    driving_performance = [driving_performance[method]['driving'] for method in methods_to_plot]
    ...
    for i, method in enumerate(methods_to_plot):
        ...
        plt.scatter(dynamic_metric[i], driving_performance[i], color='black')
        plt.annotate(remap_methods[method], (dynamic_metric[i], driving_performance[i]), 
                     textcoords="offset points", xytext=(-2,7), ha='center')

    # Add labels and legend
    plt.xlabel(f'Dynamic {metric_text_name}')
    ...
    plt.grid(True, linewidth=0.5, alpha=0.7)
    plt.savefig(f"D2_Dynamic_{metric}_vs_Driving_Performance_RVO.pdf", dpi=300)
    plt.clf()
...
if __name__ == "__main__":
    ...
    if  args.mode == "data":
        ...
        for key in directories_map_RVO.keys():
            ...
            save_dict_to_file(prediction_performance_RVO, f'pickle_filesrvo2/prediction_performance_RVO_{key}.pickle')
            save_dict_to_file(driving_performance_RVO, f'pickle_filesrvo2/driving_performance_RVO_{key}.pickle')
            save_dict_to_file(algorithm_performance_RVO, f'pickle_filesrvo2/algorithm_performance_RVO_{key}.pickle')
    ...
    # Done
    #plot_dynamic_vs_staticade_scatter(prediction_performance_RVO, list(prediction_performance_RVO.keys()))
