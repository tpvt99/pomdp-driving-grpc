...
def parse_data(txt_file):
    ...
    return action_list, ego_list, ego_path_list, exos_list, coll_bool_list, \
        pred_car_list, pred_exo_list, trial_list, depth_list, expanded_nodes, total_nodes
...
def ade_fde(pred_car_list, pred_exo_list, ego_list, exos_list):
    ...
    for timestep in range(len(exos_list.keys())):
        ...
        for agent_index, exo_agent in enumerate(exos_list[timestep]):
            ...
            for j in range(num_pred):
                ...
                all_agent_ids_at_prev_20_timestep = [agent['id'] for agent in exos_list[timestep-20]]
    ...
    return np.mean(ego_ade), np.mean(ego_fde), exo_ade, exo_fde
