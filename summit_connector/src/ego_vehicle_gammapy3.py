...
print("Hihi I am here in EgoVehicleGamma3")
print(sys.version)
...
class EgoVehicle(Summit):
    def __init__(self):
        ...
        moped_pyro_port = rospy.get_param('~moped_pyro_port', '8200')
...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():
        ...
        uri = "PYRO:motion_prediction_service@localhost:5001"  # Replace server_ip with the actual IP address of the server
        motion_prediction_service = Pyro4.Proxy(uri)

        data = {
            'key': 20,  # Replace with the actual input data required by the motion prediction service
        }

        result = motion_prediction_service.predict(data)
        print("Receiving back data ", result)
