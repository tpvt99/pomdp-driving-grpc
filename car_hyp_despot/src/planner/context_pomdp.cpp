...
SolverPrior *ContextPomdp::CreateSolverPrior(World *world, std::string name,
    ...
    if (MopedParams::PHONG_DEBUG)
        logi << "[PHONG] ContextPomdp::CreateSolverPrior 12" << endl;
...
std::vector<State *> ContextPomdp::ConstructParticles(
    ...
    if (MopedParams::PHONG_DEBUG)
        logi << "[PHONG] ContextPomdp::ConstructParticles 12" << endl;
...
bool ContextPomdp::Step(State& state_, double rNum, int action, double &reward,
    ...
    // This function will change state_, reward, and obs. Therefore any function call Step() will
    // mutately change the state_, reward and obs
    ...
    if (MopedParams::PHONG_DEBUG) {
        logi << "[Phong] ContextPomdp::Step 123 - after: " << " action: " << action << "rewards: " << reward << "obs: " << obs << endl;
        //state.PomdpStateText(cout);
    }
...
bool ContextPomdp::Step(PomdpStateWorld &state, double rNum, int action,
    ...
    if (MopedParams::PHONG_DEBUG)
        logi << "[PHONG] ContextPomdp::Step 987" << endl;
...
PomdpState* ContextPomdp::PredictAgents(const PomdpState *ped_state, int acc) const {
    // To prevent ped_state changing
    PomdpState* predicted_state = static_cast<PomdpState*>(Copy(ped_state));

    if (MopedParams::PHONG_DEBUG) {
        logi << "[PHONG] ContextPomdp::PredictAgents 123" << endl;
    }
    ...
    if (MopedParams::PHONG_DEBUG) {
        logi << "[Phong] ContextPomdp::PredictAgents - before: " << " action: " << action << " rewards: " << dummy_reward << " obs: " << dummy_obs << endl;
        //predicted_state->PomdpStateText(cout);
    }

    // Predicted_state, dummy_reward and dummy_obs will be changed
    // history of agents and car in predicted_state will be also changed

    bool terminal = Step(*predicted_state, rNum, action, dummy_reward, dummy_obs);

    // Do this to prove that Step() will change predicted_state, dummy_reward and dummy_obs
    if (MopedParams::PHONG_DEBUG) {
        logi << "[Phong] ContextPomdp::PredictAgents - after: " << " action: " << action << " rewards: " << dummy_reward << " obs: " << dummy_obs << endl;;
        //predicted_state->PomdpStateText(cout);
    }
...
Belief *ContextPomdp::InitialBelief(const State *state, string type) const {
    if (MopedParams::PHONG_DEBUG)
        logi << "[PHONG] ContextPomdp::InitialBelief" << endl;
