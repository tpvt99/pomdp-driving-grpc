...
void WorldModel::AgentStep(AgentStruct &agent, double& random) {
    if (MopedParams::PHONG_DEBUG) {
        logi << "[PHONG] WorldModel::AgentStep (agent, rand) agent_info before: ";
        agent.PhongAgentText(cout);
    }
    ...
    if (MopedParams::PHONG_DEBUG) {
        logi << "[PHONG] WorldModel::AgentStep (agent, rand) agent_info after: ";
        agent.PhongAgentText(cout);
    }
}
...
void WorldModel::GammaAgentStep(AgentStruct& agent, int intention_id) {
    if (MopedParams::PHONG_DEBUG) {
        logi << "[PHONG] WorldModel::GammaAgentStep before: add: " << &agent << " ";
        agent.PhongAgentText(cout);
    }
    ...
    if (MopedParams::PHONG_DEBUG) {
        logi << "[PHONG] WorldModel::GammaAgentStep after: ";
        agent.PhongAgentText(cout);
    }
}
...
void WorldModel::GammaAgentStep(AgentStruct agents[], double& random,
    ...
    if (MopedParams::PHONG_DEBUG) {
        logi << "[PHONG] WorldModel::GammaAgentStep [123]" << endl;
    }
...
void WorldModel::PhongAgentStep(AgentStruct &agent, double& random,  std::map<int, std::vector<double>> result) {
    // Method is called in Context_POMDP::Step()
...
void WorldModel::AgentStepCurVel(AgentStruct& agent, int step, double noise) {
    ...
    if (MopedParams::PHONG_DEBUG) {
        logi << "[PHONG] WorldModel::AgentStepCurVel agent info after: ";
        agent.PhongAgentText(cout);
        logi << endl;
    }
}
...
void WorldModel::AgentStepPath(AgentStruct& agent, int step, double noise,
                               bool doPrint) {
    if (MopedParams::PHONG_DEBUG) {
        logi << "[PHONG] WorldModel::AgentStepPath [d] agent before:" << endl;
        agent.PhongAgentText(cout);
        logi << endl;
    }
    ...
    if (MopedParams::PHONG_DEBUG) {
        logi << "[PHONG] WorldModel::AgentStepPath [d] agent after:" << endl;
        agent.PhongAgentText(cout);
        logi << endl;
    }
}
...
void WorldModel::PedStepPath(AgentStruct& agent, int step, double noise,
                             bool doPrint) {
    if (MopedParams::PHONG_DEBUG) {
        logi << "[PHONG] WorldModel::PedStepPath [x] before:" << endl;
        agent.PhongAgentText(cout);
        logi << endl;
    }
    ...
    if (MopedParams::PHONG_DEBUG) {
        logi << "[PHONG] WorldModel::PedStepPath after:" << endl;
        agent.PhongAgentText(cout);
        logi << endl;
    }
}
...
void WorldModel::VehStepPath(AgentStruct& agent, int step, double noise,
                             bool doPrint) {
    if (MopedParams::PHONG_DEBUG) {
        logi << "[PHONG] WorldModel::VehStepPath [q] before:" << endl;
        agent.PhongAgentText(cout);
        logi << endl;
    }
    ...
    if (MopedParams::PHONG_DEBUG) {
        logi << "[PHONG] WorldModel::VehStepPath after:" << endl;
        agent.PhongAgentText(cout);
        logi << endl;
    }
}
...
void WorldModel::RobStep(CarStruct &car, double steering, Random& random) {
    if (MopedParams::PHONG_DEBUG) {
        logi << "[PHONG] WorldModel::RobStep(car, random, acc)--33 before:" << endl;
    }

    BicycleModel(car, steering, car.vel);

    if (MopedParams::PHONG_DEBUG) {
        logi << "[PHONG] WorldModel::RobStep(car, random, acc)--33 after:" << endl;
    }
}

void WorldModel::RobStep(CarStruct &car, double steering, double& random) {
    if (MopedParams::PHONG_DEBUG) {
        logi << "[PHONG] WorldModel::RobStep(car, random, acc)--44 before:" << endl;
    }

    BicycleModel(car, steering, car.vel);

    if (MopedParams::PHONG_DEBUG) {
        logi << "[PHONG] WorldModel::RobStep(car, random, acc)--44 after:" << endl;
    }
}
...
void WorldModel::RobStep(CarStruct &car, double& random, double acc,
                         double steering) {
    if (MopedParams::PHONG_DEBUG) {
        logi << "[PHONG] WorldModel::RobStep(car, random, acc)--11 before:" << endl;
    }
    ...
    if (MopedParams::PHONG_DEBUG) {
        logi << "[PHONG] WorldModel::RobStep(car, random, acc)--11 after:" << endl;
    }
}
...
void WorldModel::RobStep(CarStruct &car, Random& random, double acc,
    ...
    if (MopedParams::PHONG_DEBUG) {
        logi << "[PHONG] WorldModel::RobStep(car, random, acc)--77 before:" << endl;
    }
    ...
    if (MopedParams::PHONG_DEBUG) {
        logi << "[PHONG] WorldModel::RobStep(car, random, acc)--77 after:" << endl;
    }
}
...
void WorldModel::RobVelStep(CarStruct &car, double acc, Random& random) {
    ...
    if (MopedParams::PHONG_DEBUG) {
        logi << "[PHONG] WorldModel::RobVelStep(car, random, acc)--88 before:" << endl;
    }
    ...
    if (MopedParams::PHONG_DEBUG) {
        logi << "[PHONG] WorldModel::RobVelStep(car, random, acc)--88 after:" << endl;
    }
...
void WorldModel::RobVelStep(CarStruct &car, double acc, double& random) {
    if (MopedParams::PHONG_DEBUG) {
        logi << "[PHONG] WorldModel::RobVelStep(car, random, acc)--99 before:" << endl;
    }
    const double N = ModelParams::NOISE_ROBVEL;
    if (N > 0) {
        if (FIX_SCENARIO != 1 && !CPUDoPrint)
    ...
    car.vel = max(min(car.vel, ModelParams::VEL_MAX), 0.0);
    if (MopedParams::PHONG_DEBUG) {
        logi << "[PHONG] WorldModel::RobVelStep(car, random, acc)--99 after:" << endl;
    }
    return;
}
...
void WorldModel::RobStepCurVel(CarStruct &car) {
    if (MopedParams::PHONG_DEBUG) {
        logi << "[PHONG] WorldModel::RobStepCurVel(car)--xx before:" << endl;
    }
    ...
    if (MopedParams::PHONG_DEBUG) {
        logi << "[PHONG] WorldModel::RobStepCurVel(car)--xx after:" << endl;
    }
}

void WorldModel::RobStepCurAction(CarStruct &car, double acc, double steering) {
    if (MopedParams::PHONG_DEBUG) {
        logi << "[PHONG] WorldModel::RobStepCurAction(car) [yy] before:" << endl;
    }
    ...
    if (MopedParams::PHONG_DEBUG) {
        logi << "[PHONG] WorldModel::RobStepCurAction(car) [yy] after:" << endl;
    }
}

COORD WorldModel::GetGoalPos(const AgentStruct& agent, int intention_id) {
    if (MopedParams::PHONG_DEBUG)
        logi << "[PHONG] WorldModel::GetGoalPos" << endl;
...
COORD WorldModel::GetGoalPosFromPaths(int agent_id, int intention_id,
    ...
    if (MopedParams::PHONG_DEBUG)
        logi << "[PHONG] WorldModel::GetGoalPosFromPaths" << endl;
...
void WorldModel::BicycleModel(CarStruct &car, double steering, double end_vel) {
    //logi << "[PHONG] WorldModel::BicycleModel(agent, steering, end_vel)--11:" << endl;
...
void WorldModel::BicycleModel(AgentStruct &agent, double steering,
                              double end_vel) {
    //logi << "[PHONG] WorldModel::BicycleModel(agent, steering, end_vel)--22:" << endl;
...
void WorldModel::AddEgoGammaAgent(int id_in_sim, const CarStruct& car) {
    ...
    //logi << "[PHONG] WorldModel::AddEgoGammaAgent:" << endl;
...
void WorldModel::PrepareAttentiveAgentMeanDirs(const State* state) {
    logi << "[PHONG] WorldModel::PrepareAttentiveAgentMeanDirs 4.a(1) Beginning: " << endl;
    ...
    logi << "[PHONG] WorldModel::PrepareAttentiveAgentMeanDirs 4.a(2) num_agents in belief tracker: " << num_agents << endl;

    if (num_agents == 0)
        return;

    logi << "[PHONG] WorldModel::EnsureMeanDirExist 4.a(2) Beginning: " << endl;
    ...
    if (initial_update_step){
        logi << "[PHONG] WorldModel::PrepareAttentiveAgentMeanDirs 4.a(3) Initial Update: " << endl;
        ...
        int threadID = GetThreadID();
        logi << "[PHONG] WorldModel::PrepareAttentiveAgentMeanDirs 4.a(4) Gamma thread clear agents and add new agent: " << endl;

        traffic_agent_sim_[threadID]->clearAllAgents();

        std::vector<int> agent_ids;
        int gamma_id = 0;
        logi << "[PHONG] WorldModel::PrepareAttentiveAgentMeanDirs 4.a(5) Add agent gamma_id" << endl;
        ...
        logi << "[PHONG] WorldModel::PrepareAttentiveAgentMeanDirs 4.a(6) simulate agent: agent intention gamma_id | ";
        ...
        for (int i = 0; i < crowd_state->num; i++) {
            ...
            for (int intention_id = 0; intention_id < GetNumIntentions(agent_id);
                ...
                logi << "|" << agent_id << " " << intention_id << " " << gamma_id << " ";
